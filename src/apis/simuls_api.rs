...
/// struct for typed errors of method [`api_simul`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiSimulError {
...
pub async fn api_simul(configuration: &configuration::Configuration, ) -> Result<crate::models::ApiSimul200Response, Error<ApiSimulError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/simul", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ApiSimulError> = serde_json::from_str(&local_var_content).ok();
