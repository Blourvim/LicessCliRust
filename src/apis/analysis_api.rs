...
/// struct for typed errors of method [`api_cloud_eval`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiCloudEvalError {
...
pub async fn api_cloud_eval(configuration: &configuration::Configuration, fen: &str, multi_pv: Option<f32>, variant: Option<VariantKey>) -> Result<(), Error<ApiCloudEvalError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/cloud-eval", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());

    local_var_req_builder = local_var_req_builder.query(&[("fen", &fen.to_string())]);
    if let Some(ref local_var_str) = multi_pv {
        local_var_req_builder = local_var_req_builder.query(&[("multiPv", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ApiCloudEvalError> = serde_json::from_str(&local_var_content).ok();
