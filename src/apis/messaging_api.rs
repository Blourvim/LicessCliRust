...
/// struct for typed errors of method [`inbox_username`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum InboxUsernameError {
...
pub async fn inbox_username(configuration: &configuration::Configuration, username: &str, text: &str) -> Result<crate::models::Ok, Error<InboxUsernameError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/inbox/{username}", local_var_configuration.base_path, username=crate::apis::urlencode(username));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<InboxUsernameError> = serde_json::from_str(&local_var_content).ok();
